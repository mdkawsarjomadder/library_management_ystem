...
page.Header().Column(column =>
{
    column.Item().Row(row =>
    {
        ...
        row.RelativeItem()
            .AlignRight()
            .Text($"Date: {DateTime.Now:dd MMM yyyy, ddd / dddd}")
            .FontSize(11);
    });
...
{
    row.RelativeItem()
        .Text($"Generated: {DateTime.Now:dd MMM yyyy HH:mm:ss}")
        .FontSize(9);
        ...
        .Text(text =>
        {
            ...
            text.CurrentPageNumber().FontSize(9);
                    ...
                    page.Content().Table(table =>
                    {
                        ...
                        table.Header(header =>
                            {
                                header.Cell().Border(1).Background("#D9EAD3").Padding(5).Text("Member").Bold();
                                header.Cell().Border(1).Background("#D9EAD3").Padding(5).Text("Book").Bold();
                                header.Cell().Border(1).Background("#D9EAD3").Padding(5).Text("Issue Date").Bold();
                                header.Cell().Border(1).Background("#D9EAD3").Padding(5).Text("Due Date").Bold();
                                header.Cell().Border(1).Background("#D9EAD3").Padding(5).Text("Return Date").Bold();
                                header.Cell().Border(1).Background("#D9EAD3").Padding(5).Text("Status").Bold();
                                header.Cell().Border(1).Background("#D9EAD3").Padding(5).Text("Fine").Bold();
                            });

                        foreach (var item in issueBooks)
                        {
                            table.Cell().Border(1).Padding(5).Text(item.User?.Name ?? "");
                            table.Cell().Border(1).Padding(5).Text(item.Book?.Title ?? "");
                            table.Cell().Border(1).Padding(5).Text(item.IssueDate.ToString("dd MMM yyyy"));
                            table.Cell().Border(1).Padding(5).Text(item.DueDate.ToString("dd MMM yyyy"));
                            table.Cell().Border(1).Padding(5).Text(item.ReturnDate?.ToString("dd MMM yyyy") ?? "-");
                            table.Cell().Border(1).Padding(5).Text(item.IsReturned ? "Returned" : "Issued");
                            table.Cell().Border(1).Padding(5).Text($"৳ {item.Fine}");
                        }
                    });
